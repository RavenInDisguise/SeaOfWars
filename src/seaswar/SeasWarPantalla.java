...
public class SeasWarPantalla extends javax.swing.JFrame {
    ...
    public void pintarCeldasVivas(int i, int j, int vida, String color){
        if(vida==1){
            butt[i+1][j+1].setBackground(Color.magenta);
        }else {
            butt[i+1][j+1].setBackground(Color.RED);
        }
    ...
    public void log(){
        try {
            refCliente.hiloCliente.writer.writeInt(17);
        } catch (IOException ex) {
            Logger.getLogger(SeasWarPantalla.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
    
    public void consultarEnemigo(String nombre){
        try {
    ...
    public void actualizarTotal(int vivasActuales){
        int vida=(vivasActuales*100)/600;
        int destruidas = 600-vivasActuales;
        lbl_vidaGen.setText("Vida: "+vida);
        lbl_casillasDestGen.setText("Casillas destruidas: "+destruidas);
    }
    
    public void mostrarPorcentajeCeldas(int i, int j, String porcentaje){
        System.out.println("porcentaje de celdas");
        Font fuente = new Font("Arial", 3, 10);
        butt[i+1][j+1].setFont(fuente);
        butt[i+1][j+1].setText("100");
    }
    
    public void porcentajeCeldas(){
        try {
            refCliente.hiloCliente.writer.writeInt(19);
        } catch (IOException ex) {
            Logger.getLogger(SeasWarPantalla.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
    
    public void mostrarCeldasVivas(){
        try {
    ...
    private void initComponents() {
        ...
        lbl_casillasDestGen.setText("Casillas destruidas: 0");

        lbl_vidaGen.setFont(new java.awt.Font("Tahoma", 0, 24)); // NOI18N
        lbl_vidaGen.setText("Vida: 100");
        ...
        if(evt.getKeyCode()==10){
            ...
            if(datos[0].trim().equals("CREARPERSONAJE")){
                ...
                enviarCaracteristica();
                 ...
                 consultarCelda();   
            }else if(datos[0].trim().equals("LOG")){   
                 log(); 
            }else if(datos[0].trim().equals("CONSULTARENEMIGO")){   
                 consultarEnemigo(datos[1].trim().toUpperCase());
            }else if(datos[0].trim().equals("PORCENTAJECELDAS")){   
                 porcentajeCeldas();
            }else if(datos[0].trim().equals("PINTARVIVAS")){   
                 mostrarCeldasVivas();
            }else{
                try {
                    refCliente.hiloCliente.writer.writeInt(4);
